...
- (void)setImage:(UIImage *)image
{
    ...
    if (_quadrateImage) {
        ...
        CGRect requiredBounds = CGRectZero;
        if (imageSize.width>=imageSize.height) {
            requiredBounds = CGRectMake((image.size.width-image.size.height)/2,
                                0,
                                image.size.height,
                                image.size.height);
        } else {
            requiredBounds = CGRectMake(0,
        ...
        CGImageRef imageRef = CGImageCreateWithImageInRect([image CGImage], requiredBounds);
        UIImage *croppedImage = [UIImage imageWithCGImage:imageRef];
        CGImageRelease(imageRef);
        
        super.image = croppedImage;
    } else {
        super.image = image;
    }
